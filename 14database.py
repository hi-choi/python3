...
# 데이터베이스 연결
import sqlite3
conn = sqlite3.connect('d:/sqllite3/sqlite-tools-win32-x86-3360000/siestageek.db')

# 접속후 커서를 가져옴
cur = conn.cursor()

# 질의문 ㅈ가성
sql = 'select pcode, pname, price, amount from productTable'
...
# 리스트 내용 출력 2
# 리스트를 컬럼명으로 리스트를 출력하려면 커서모드는 dictCursor 이용
# 단, sqlite3은 미지원
# for row in rows :
...
sql2 = ' select * from EMPLOYEES where JOB_ID = "IT_PROG" '
cursor.execute(sql2)
...
sql3 = ' select count(EMPLOYEE_ID) cnt from EMPLOYEES where DEPARTMENT_ID = 30 '
cursor.execute(sql3)
...
price = 35000
amount = 10
sql = ' insert into productTable values ' \
      f'("{pcode}","{pname}",{price},{amount})'

# 질의문 작성 2 - sqlite3 미지원
sql = ' insert into productTable values (%s, %s, %s, %s) '
