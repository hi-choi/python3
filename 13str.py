...
str.count('공부') # 특정문자열 존재 갯수
str.find('공부') # 문자열을 찾은 위치 (왼쪽 기준)
str.find('공부',5) # 특정 인덱스를 시작으로 문자열을 찾은 위치 (왼쪽 기준)
...
str.index('공부',5)
str.rindex('공부')
str.index('없다') # 찾지 못하는 경우 : 오류발생!

str.startswith('파이썬') # 특정 문자열로 시작하는지 여부
str.startswith('파이썬',10)
str.startswith('물론',str.find('물론'))
...
# 문자열 분리 결합
str = 'Lisa,98,65,77' # 구분기호로 문자열을 분리하고 리스트로 저장
str.split(',')
